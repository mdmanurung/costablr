...
- `stablr-intro.Rmd` is the quick/easy vignette: simulated data with compact
  lambda grids and 40-50 bootstraps.
- Real-data vignettes remain realistic but bounded:
  - OOL multi-omic: 80 bootstraps, 15 lambda values, 500 late-fusion iterations.
  - Python-to-R mapping: OOL rendered with 120 bootstraps and random-permutation
    decoys; high-fidelity knockoff parity is documented for extended runs.
