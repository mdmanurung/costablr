# visualization.R — ggplot2 diagnostic and result plots for costablr
...
plot_stabl_path <- function(object, new_hard_threshold = NULL,
  ...
  if (!is.null(df_art)) {
    p <- p + ggplot2::geom_line(
      ...
      colour  = .costablr_colors$artificial,
      alpha   = 0.4,
      linewidth = 0.4,
      linetype = "dotted"
    )
  ...
  if (nrow(df_sel) > 0L) {
    p <- p + ggplot2::geom_line(
      data    = df_sel,
      mapping = ggplot2::aes(x = lambda_idx, y = score, group = feature),
      colour  = .costablr_colors$selected,
      alpha   = 1,
      linewidth = 1
    )
  }
  ...
  if (!is.null(threshold)) {
    ...
    p <- p + ggplot2::geom_hline(
      ...
      linetype   = "dashed",
      colour     = .costablr_colors$threshold,
      linewidth  = 0.7
    ) + ggplot2::annotate(
      "text",
      x     = Inf, y = threshold,
      ...
      size  = 3, colour = .costablr_colors$threshold
    )
  }
...
plot_fdr_graph <- function(object, title = "FDR Estimate", fdr_target = 0.05) {
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = threshold, y = FDR)) +
    ggplot2::geom_line(colour = .costablr_colors$noise, linewidth = 1) +
    ggplot2::geom_vline(
...
plot_roc <- function(y_true, y_preds, title = "ROC Curve") {
  ...
  ggplot2::ggplot(roc_df, ggplot2::aes(x = fpr, y = tpr)) +
    ggplot2::geom_abline(
      slope     = 1, intercept = 0,
      linetype  = "dashed",
      colour    = .costablr_colors$chance,
      linewidth = 0.8,
      alpha     = 0.7
    ) +
    ggplot2::geom_line(colour = .costablr_colors$selected, linewidth = 1.2) +
...
scatterplot_features <- function(features, x, y, title = "Selected Features", ncol = 3L) {
  ...
  ggplot2::ggplot(df, ggplot2::aes(x = value, y = outcome)) +
    ggplot2::geom_point(
      colour = .costablr_colors$noise, alpha = 0.6, size = 1
    ) +
    ggplot2::geom_smooth(
      method  = "loess",
      formula = y ~ x,
