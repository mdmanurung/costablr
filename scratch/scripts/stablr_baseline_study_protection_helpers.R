# Shared helpers for the AURORA baseline study-by-protection STABL analysis.
...
baseline_config <- function() {
  ...
  target_levels <- c("EG_P", "EG_NP", "TU_P", "TU_NP", "GA_P", "GA_NP")
  list(
    ...
    sample_fraction = as.numeric(Sys.getenv("STABLR_SAMPLE_FRACTION", "0.9")),
    ...
    cache_dir = Sys.getenv(
      "STABLR_CACHE_DIR",
      file.path("scratch", "cache", "stablr_baseline_study_protection_test")
    ),
    export_dir = Sys.getenv(
      "STABLR_EXPORT_DIR",
      file.path("scratch", "outputs", "stablr_baseline_study_protection_test")
    ),
    study_group_map = c(CVTU3 = "TU", EGSV2 = "EG", PfGA2 = "GA"),
    study_arm_levels = c("EG", "GA", "TU"),
    study_group_levels = target_levels,
    ...
    nested_outer_v = as.integer(Sys.getenv("STABLR_NESTED_OUTER_V", "3")),
    nested_inner_v = as.integer(Sys.getenv("STABLR_NESTED_INNER_V", "2")),
    cooperative_nfolds = as.integer(Sys.getenv("STABLR_COOPERATIVE_NFOLDS", "3"))
  )
}
...
add_analysis_labels <- function(meta, config) {
  ...
  meta$study_arm <- unname(config$study_group_map[as.character(meta$study_id)])
  meta$study_arm <- factor(meta$study_arm, levels = config$study_arm_levels)
  crossed <- ifelse(
    !is.na(meta$study_arm) & !is.na(meta$protection),
    paste(as.character(meta$study_arm), as.character(meta$protection), sep = "_"),
    NA_character_
  )
  meta$study_protection_group <- factor(crossed, levels = config$study_group_levels)
...
preprocess_all_views <- function(config, force = FALSE) {
  ...
  source_preprocess_cache <- Sys.getenv(
    "STABLR_SOURCE_PREPROCESS_CACHE",
    file.path("scratch", "cache", "stablr_baseline_groups_test", "preprocess", "baseline_preprocessed.rds")
  )
  input_paths <- c(unname(config$all_view_paths), config$metadata_path)
  if (!all(file.exists(input_paths)) && file.exists(source_preprocess_cache)) {
    message(
      "Raw input files are unavailable; deriving six-label preprocessing cache from existing all-view cache: ",
      source_preprocess_cache
    )
    source_cache <- readRDS(source_preprocess_cache)
    if (!all(c("x_list", "metadata") %in% names(source_cache))) {
      stop("Unsupported source preprocessing cache format: ", source_preprocess_cache, call. = FALSE)
    }
    metadata_baseline <- as.data.frame(source_cache$metadata, check.names = FALSE)
    if (!"protection" %in% names(metadata_baseline)) {
      stop("Source preprocessing cache lacks `protection` metadata.", call. = FALSE)
    }
    if ("study_group" %in% names(metadata_baseline) &&
        all(as.character(metadata_baseline$study_group) %in% config$study_arm_levels)) {
      metadata_baseline$study_arm <- as.character(metadata_baseline$study_group)
    } else if (!"study_arm" %in% names(metadata_baseline)) {
      stop("Source preprocessing cache lacks a mappable study-arm column.", call. = FALSE)
    }
    crossed <- ifelse(
      !is.na(metadata_baseline$study_arm) & !is.na(metadata_baseline$protection),
      paste(as.character(metadata_baseline$study_arm), as.character(metadata_baseline$protection), sep = "_"),
      NA_character_
    )
    metadata_baseline$study_arm <- factor(metadata_baseline$study_arm, levels = config$study_arm_levels)
    metadata_baseline$protection <- factor(metadata_baseline$protection, levels = c("P", "NP"))
    metadata_baseline$study_protection_group <- factor(crossed, levels = config$study_group_levels)
    metadata_baseline$study_group <- metadata_baseline$study_protection_group
    if (anyNA(metadata_baseline$study_protection_group)) {
      stop("Source preprocessing cache contains samples without crossed study/protection labels.", call. = FALSE)
    }
    common_ids <- rownames(metadata_baseline)
    x_list <- lapply(source_cache$x_list, function(x) {
      x <- as.matrix(x)
      x[common_ids, , drop = FALSE]
    })
    if (any(!vapply(x_list, function(x) identical(rownames(x), common_ids), logical(1L)))) {
      stop("Source preprocessing cache sample alignment failed.", call. = FALSE)
    }
    y <- stats::setNames(metadata_baseline$study_protection_group, common_ids)
    baseline_class_qc <- tibble::tibble(study_protection_group = y) |>
      dplyr::count(.data$study_protection_group, name = "n") |>
      tidyr::complete(
        study_protection_group = factor(config$study_group_levels, levels = config$study_group_levels),
        fill = list(n = 0L)
      )
    baseline_arm_protection_qc <- metadata_baseline |>
    ...
    out <- list(
      ...
      analysis_samples = source_cache$analysis_samples %||% common_ids,
      nzv_summary = source_cache$nzv_summary %||% tibble::tibble(),
      scaling_qc = source_cache$scaling_qc %||% tibble::tibble(),
      baseline_class_qc = baseline_class_qc,
      baseline_arm_protection_qc = baseline_arm_protection_qc,
      source_preprocess_cache = normalizePath(source_preprocess_cache, winslash = "/", mustWork = FALSE)
    )
    cache_object(out, "baseline_preprocessed", "preprocess", config)
    if (nrow(out$nzv_summary) > 0L) export_table(out$nzv_summary, "near_zero_variance_qc", "preprocess", config)
    if (nrow(out$scaling_qc) > 0L) export_table(out$scaling_qc, "scaling_qc", "preprocess", config)
    export_table(out$baseline_class_qc, "baseline_class_qc", "preprocess", config)
    export_table(out$baseline_arm_protection_qc, "baseline_arm_protection_qc", "preprocess", config)
  ...
  eligible <- rownames(metadata)[metadata$timepoint == "t1" & !is.na(metadata$study_protection_group)]
...
per_class_metric_table <- function(truth, predicted, levels = NULL) {
  levels <- levels %||% levels(factor(truth))
  truth <- factor(truth, levels = levels)
  predicted <- factor(predicted, levels = levels)
  confusion <- table(truth = truth, predicted = predicted)
...
sample_cluster_purity <- function(mat, metadata, k = 6L) {
  if (ncol(mat) < k) return(tibble::tibble())
  hc <- stats::hclust(stats::dist(t(mat)))
  clusters <- stats::cutree(hc, k = k)
  tab <- as.data.frame(table(
    cluster = clusters,
    study_protection_group = metadata[names(clusters), "study_group"]
  ))
  tab |>
...
run_late_fusion_branch <- function(data, config) {
  ...
  export_table(
    per_class_metric_table(pred_tbl$truth, pred_tbl$predicted_class, levels = config$study_group_levels),
...
.majority_prediction <- function(y, n) {
  majority <- names(which.max(table(y)))
  rep(majority, n)
}

predict_binomial_selected_features <- function(fit_bundle, x, y) {
  levels <- levels(y)
  selected <- get_feature_names_out(fit_bundle$fit)
  if (length(selected) == 0L || !requireNamespace("glmnet", quietly = TRUE)) {
    predicted <- .majority_prediction(y, nrow(x))
    return(tibble::tibble(
      sample_id = rownames(x),
      truth = as.character(y),
      predicted_class = predicted,
      positive_probability = NA_real_,
      n_selected_features = length(selected),
      prediction_mode = "majority_fallback"
    ))
  }
  nfolds <- max(2L, min(3L, min(table(y))))
  pred <- tryCatch({
    ...
    cls <- as.character(stats::predict(cvfit, newx = x[, selected, drop = FALSE], s = "lambda.min", type = "class")[, 1L])
    prob <- as.numeric(stats::predict(cvfit, newx = x[, selected, drop = FALSE], s = "lambda.min", type = "response")[, 1L])
    list(class = cls, probability = prob, mode = "selected_feature_glmnet")
  }, error = function(e) {
    list(
      class = .majority_prediction(y, nrow(x)),
      probability = rep(NA_real_, nrow(x)),
      mode = paste("majority_fallback:", conditionMessage(e))
    )
  })
  tibble::tibble(
    sample_id = rownames(x),
    ...
    positive_probability = pred$probability,
    n_selected_features = length(selected),
    prediction_mode = pred$mode
  )
}

run_ovr_stabl_branch <- function(data, config, group) {
  x_early <- do.call(cbind, purrr::imap(data$x_list, prefix_feature_names))
  y_bin <- factor(ifelse(as.character(data$y) == group, group, "rest"), levels = c("rest", group))
  names(y_bin) <- names(data$y)
  bootstrap_strata <- data.frame(
    outcome = y_bin,
    study_protection_group = data$y,
    row.names = names(data$y)
  )
  branch <- paste0("ovr_stabl/", group)
  fit_bundle <- fit_stabl_branch(
    x = x_early,
    y = y_bin,
    config = config,
    branch = branch,
    family = "binomial",
    bootstrap_strata = bootstrap_strata,
    seed_offset = 10000L + match(group, config$study_group_levels) * 1000L
  )
  pred_tbl <- predict_binomial_selected_features(fit_bundle, x_early, y_bin)
  export_table(pred_tbl, "ovr_stabl_train_predictions", branch, config)
  export_table(confusion_table(pred_tbl$truth, pred_tbl$predicted_class, levels = levels(y_bin)),
               "ovr_stabl_confusion_matrix", branch, config)
  export_table(classification_metric_table(pred_tbl$truth, pred_tbl$predicted_class, levels = levels(y_bin)),
               "ovr_stabl_metrics", branch, config)
  export_table(per_class_metric_table(pred_tbl$truth, pred_tbl$predicted_class, levels = levels(y_bin)),
               "ovr_stabl_per_class_metrics", branch, config)
  fit_bundle
}
...
run_cooperative_ovr_branch <- function(data, config, group) {
  ...
  fit <- stabl_multiomic_train_validate(
    ...
    cooperation_nfolds = config$cooperative_nfolds
  )
