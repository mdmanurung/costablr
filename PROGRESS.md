...
- Refreshed the five-vignette source set to balance realistic examples with
  ...
  - `stablr-intro.Rmd` is now the explicit quick-start vignette using smaller
    simulated data, compact lambda grids, and 40-50 bootstrap examples.
  - `stablr-multiomic.Rmd` keeps the real OOL train/validation workflow but
    uses 80 bootstraps, 15-point lambda grids, and a 500-iteration late-fusion
    search.
  - `stablr-python-parity.Rmd` is now a bounded Python-to-R workflow mapping:
  ...
  - `stablr-cooperative.Rmd` keeps real OOL cooperative fusion but uses compact
    lambda/rho/fold settings and marks outer cooperative CV as displayed but
    not evaluated during vignette builds.
  - Restored canonical `r-pkg/stablr/vignettes/stablr-tcga.Rmd` source for the
    TCGA breast-cancer vignette, replacing stale generated source artifacts in
    `vignettes/`.
- Clarified source-vs-generated vignette layout in `r-pkg/stablr/README.md`:
...
#    stablr-cooperative, stablr-intro, stablr-multiomic,
...
- Rewrote `r-pkg/stablr/README.md` to reflect the current package state:
...
- Updated package metadata in `r-pkg/stablr/DESCRIPTION`:
...
- Updated package-level roxygen docs in `R/input_validation.R` and regenerated
  Rd help. `stablr-package.Rd` now describes main workflows, learners,
...
- Updated multi-omic workflow titles/docs from "Minimal" to current public
  ...
  fusion docs.
- Added `r-pkg/stablr/docs/API_REFERENCE.md` as a human-readable exported API
  map.
- Added `r-pkg/stablr/_pkgdown.yml` with grouped reference sections and
  vignette navigation; pkgdown output is configured to `docs/stablr`.
- Built the pkgdown documentation website in `docs/stablr`.

Validation:

```bash
conda run -n R4_51 Rscript -e "roxygen2::roxygenise('r-pkg/stablr')"
...
# -> built stablr_0.0.0.9000.tar.gz
