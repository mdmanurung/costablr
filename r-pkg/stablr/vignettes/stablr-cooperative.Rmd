...
In `stablr`, cooperative fusion uses the optional `multiview` package.  Each
...
`cooperative_fusion = TRUE`, `stablr` stops immediately with an informative
...
the rest of `stablr` works without `multiview`.
