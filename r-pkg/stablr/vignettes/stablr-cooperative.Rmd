...
with days-to-onset-of-labor (DOS) as the continuous regression target.  The
examples use compact grids so the vignette remains buildable; enlarge the
bootstrap count and `rho` grid for a final analysis.
...
`auto_lambda_grid()` and pass them as a named list.  Twelve lambda values are
enough to demonstrate the workflow without making each cooperative fit too
expensive.
...
This is a realistic analysis pattern, but it multiplies the cost by the number
of outer folds, so the code is shown but not evaluated during vignette builds:
