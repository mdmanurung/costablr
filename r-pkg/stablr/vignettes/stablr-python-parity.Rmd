...
the same preprocessing shape, bootstrap counts, lambda-grid size, artificial
feature strategy, and random seed as the notebook sections reproduced below.
Package builds without the tutorial data fall back to the bundled OOL subset
and skip the COVID-19 section.
...
- With the repository tutorial data available, the OOL run recovers the Python
  ...
  R-specific features because `glmnet` and scikit-learn use different solvers.
